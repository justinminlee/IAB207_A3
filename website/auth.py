...
#         new_user = User(
#             username=uname, firstname=fname, lastname=lname, emailid=email,
#             password_hash=pwd_hash, contact=contact_number, address=address
#         )
...
#                 nextp = url_for('main.index')  # Redirect to the home page if no 'next' page
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if error is None:
            login_user(user)
            # Ensure nextp is not None; if it is, redirect to the homepage
            nextp = request.args.get('next')
            if not nextp or not nextp.startswith('/'):
